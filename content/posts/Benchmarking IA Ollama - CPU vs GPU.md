...
El informe completo, con todas las tablas y el detalle de metodología, está disponible en [`informe-benchmarking-ia-ollama-v1.md`](https://gitlab.com/lucianodlf/inn-ia-benchmark-pub/-/blob/fcbeadbf9197a9abcfa0be3f214a4559d8192a8b/docs/informe-benchmarking-ia-ollama-v1.md).

##### Hardware de los dos entornos

| Componente | PC Local | PC Colab |
|---|---|---|
| Hostname | local-cpu-host | ed355698b496 |
| Sistema Operativo | Linux 6.16.3-76061603-generic | Linux 6.6.105+ |
| Procesador (CPU) | Intel(R) Core(TM) i7-14700 | Intel(R) Xeon(R) CPU @ 2.00GHz |
| CPU - Cores / Threads | **20 físicos** / 28 totales | 1 físico / 2 totales |
| CPU - Frecuencia Máx. | 5400.0000 MHz | — |
| Memoria RAM - Total | 31 Gi | 12 Gi |
| Memoria RAM - Velocidad | 1305MHz | — |
| GPU - Tipo/Modelo | Intel / Intel Corporation Device a780 (rev 04) | NVIDIA / Tesla T4 |
| GPU - VRAM | — | 15360 MiB |
| GPU - Driver/CUDA | — | Driver: 550.54.15 / CUDA: 12.4 |
| Aceleración Ollama | Experimental (`OLLAMA_INTEL_GPU=1`). *(Advertencia: "No se detectó GPU - Se usará CPU")* | CUDA (incluido en Ollama). Uso automático de GPU. |
| Almacenamiento (Disco raíz) | /dev/nvme0n1p3 (Tipo: HDD) | overlay (Uso: 89%) |
| Versión de Ollama | 0.12.7 | 0.12.10 |
| Variables de Ollama relevantes | `OLLAMA_NUM_PARALLEL=2`, `OLLAMA_MAX_LOADED_MODELS=2`, `OLLAMA_KV_CACHE_TYPE=q8_0`, `OLLAMA_FLASH_ATTENTION=true`, `OLLAMA_KEEP_ALIVE=0` | `OLLAMA_KEEP_ALIVE: 0` |
...
Vengo usando modelos locales con [Ollama](https://ollama.com) para distintas pruebas. Trabajando en un proyecto nos surgio la idea de automatizar la carga de comprobantes y queriamos explorar si podiamos hacerlo con los recursos que teniamos tal como lo necesitavamos. Con ese input como necesidad, me surgieron algunas preguntas: ¿tiene sentido correr modelos de visión (VLM) para tareas de OCR y extracción de datos estructurados sobre un equipo de escritorio sin GPU dedicada, o el costo en tiempo lo vuelve inviable frente a una GPU?, ¿Realmente necesitamos una GPU, o podemos resolver con buena presicion esta tarea solo con inferencia en CPU? ¿Es lo mas apropiado un modelo VLM, OCR tradicional, una combinacion de ambos?
...
Una referencia muy buena presiscion en la extraccion, un output estructura y a muy buena velocidad fue hacerlo con Claude. Suponiendo que no llegaria nunca a lograr ese mismo resultado, igual necesitabamos saber que resultado podriamos lograr.

La idea de usar una instancia de Colab, fue la forma mas rapida y gratuita de conseguir un entorno con GPU.

Armé un mini-benchmark para responder las dos preguntas a la vez: comparar hardware (CPU local vs GPU en Colab) y comparar dificultad de tarea (OCR simple vs extracción de factura). El resultado completo está en el informe del link arriba; en este post reviso la metodología, los hallazgos y los scripts que arman todo el pipeline.
...
Ejemplo simplificado de la llamada cruda a la API:
...
### Extracción de factura (T2): la tarea difícil 
...
- Probar opciones como PaddleOCR: [GitHub - PaddlePaddle/PaddleOCR](https://github.com/PaddlePaddle/PaddleOCR), o [GitHub - baidu/Unlimited-OCR:](https://github.com/baidu/Unlimited-OCR)

## Recursos

- **Repositorio del proyecto (script completo, notebooks e informe):** [gitlab.com/lucianodlf/inn-ia-benchmark-pub](https://gitlab.com/lucianodlf/inn-ia-benchmark-pub)
- **Notebook de Colab:**  Para abrir este notebook en Colab: `Archivo → Abrir notebook → GitHub/URL` pegando la [URL raw](https://gitlab.com/lucianodlf/inn-ia-benchmark-pub/-/raw/main/colab_ia_benchmark_v2.ipynb?ref_type=heads), o directamente el [notebook en el repo](https://gitlab.com/lucianodlf/inn-ia-benchmark-pub/-/blob/fcbeadbf9197a9abcfa0be3f214a4559d8192a8b/colab_ia_benchmark_v2.ipynb) para verlo primero.
- [blog.lucianodlf.com.ar](https://blog.lucianodlf.com.ar/)
